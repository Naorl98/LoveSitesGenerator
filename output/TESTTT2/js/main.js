...
function sendEmail(type, msg) {
  ...
  const toEmail      = "naorman12@gmail.com";
  const fromName     = "TESTTT";
  const replyEmail   = "naorman12@gmail.com";
  const siteTitle    = "TEST SITEEE";
  const emailMsgText = "I LOVE U! Sent automatically from my site. Date: {{date}}";
  let subject = "";
  let message = "";
  if (type === "love") {
    subject = "Love Message from " + siteTitle;
    // אם תעדיף, אפשר להחליף ל־emailMsgText עם תאריך דינמי
    message = emailMsgText.replace("{{date}}", new Date().toLocaleString());
  } else if (type === "letter") {
