// === AWS S3 CONFIG ===
const BUCKET   = "https://nessy-site.s3.eu-central-1.amazonaws.com";         // נשלח מה-main.py
const IMG_DIR  = "img/";        // נשלח מה-main.py
const SONG_DIR = "songs/";       // נשלח מה-main.py

// === MAIN DATA (הוזרק דינמית מ-main.py) ===
let galleryImages = "[]";    // מערך קבצי תמונות לגלריה
let heroBgUrl     = "";           // תמונת רקע ראשונית (או "" אם לא נבחרה)
let floatingEmoji = "😘";           // האימוג'י לפיצוץ/לבבות (ניתן לבחור ב-GUI)

// === EMAILJS CONFIG (הוזרק דינמית מ-main.py) ===
const EMAILJS_USER_ID     = "mfq1Yd_yCw97pCAJF";
const EMAILJS_SERVICE_ID  = "service_5z3zm6i";
const EMAILJS_TEMPLATE_ID = "template_ndb3wcx";
const TO_EMAIL            = "naorman12@gmail.com";
const FROM_NAME           = "NAOR";
...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  function createHearts() {
    for (let i = 0; i < 15; i++) {
      ...
      heart.textContent = floatingEmoji || "❤️"; // 👈 אפשר לבחור כל אימוג'י מה-main.py
  ...
  function sendEmail(type) {
      ...
      if (type === "love") {
        ...
        message = "I LOVE U " + ("{{date}}", new Date().toLocaleString());
      } else if (type === "letter") {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      // אם המשתמש בחר לא לטעון דינמי מה-S3, השתמש במערך שהוזן מה-main.py
      // אחרת, אם רוצים בכוח live S3, פשוט שים כאן את הקריאה ל-listS3Objects (ניתן לבחירה)
